...
**Taps** (both behaviours are settings since round 3 — see the config screen above):
- an **item** → with "Tap on an item" at its default, opens
  `dashboard.fredhli.com/#/flow/i/<id>` in the browser, with that item expanded. Your
  90-day session cookie signs you in; the widget's token has nothing to do with the
  browser. Set to *Expand in widget*, the same tap instead unfolds the item's text inline
